...
Use the centroid of Phoenix CBSA and Phoenix coordinates from OSM's website to obtain h3 polygons:

```{r}
coords <- st_centroid(phoenix_cbsa) |>
  st_coordinates()

coords <- c(33.48333, -112.073889)
```

Retrieve h3 geohashing at a fairly high resolution to identify Hamilton; the coordinates are for the City of Hamilton:

```{r}
phoenix_h3 <- geo_to_h3(c(33.48333, -112.073889),
                             res = 4) |>
  k_ring(1)
...
```{r}
phoenix_h3_sf = c("8429b69ffffffff", "8448e97ffffffff", "8429b6dffffffff", "8448ebbffffffff")
```
...
```{r}
ggplot() + 
  geom_sf(data = h3_to_geo_boundary_sf(phoenix_h3_sf),
          aes(fill = h3_index)) +
  geom_sf(data = phoenix_cbsa,
          color = "red",
          fill = NA) +
...
Obtain children at resolution 13 (average area is \~50.68 $m^2$):
